...
function getIssueStyle(severity: MarketingAuditIssue["severity"]) {
  if (severity === "high") return { border: "#fecaca", bg: "var(--danger-bg)", text: "var(--danger)", label: "Priorite haute" };
  if (severity === "medium") return { border: "#FDE68A", bg: "var(--warning-bg)", text: "var(--warning)", label: "Priorite moyenne" };
  return { border: "var(--line-strong)", bg: "var(--paper-2)", text: "var(--ink-2)", label: "Priorite basse" };
}
      ...
      <main style={{ background: "linear-gradient(180deg, var(--paper-2) 0%, #ffffff 100%)", minHeight: "100vh", padding: "124px 24px 48px" }}>
        <div style={{ maxWidth: 1180, margin: "0 auto" }}>
          {loading ? (
            ...
            <>
              {isNew ? (
                <div className="audit-banner" style={{ marginBottom: 20, padding: "14px 18px", borderRadius: 18, background: "var(--success-bg)", border: "1px solid #BBF7D0", color: "var(--success)", fontSize: 14, fontWeight: 600 }}>
              ...
              <section className="audit-shell" style={{ background: "#fff", border: "1px solid var(--line)", borderRadius: 28, padding: 28, boxShadow: "0 20px 48px rgba(15,23,42,0.06)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
                  <div style={{ maxWidth: 720 }}>
                    <p style={{ margin: "0 0 12px", fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#2A6FE8" }}>Audit de flux</p>
                    <h1 style={{ margin: "0 0 12px", fontSize: "clamp(34px, 5vw, 54px)", lineHeight: 1.02, letterSpacing: "-0.05em", color: "var(--ink)" }}>
                      {audit.company}
                    </h1>
                    <p style={{ margin: 0, fontSize: 18, lineHeight: 1.7, color: "var(--ink-2)" }}>
                  ...
                  <div className="audit-actions" style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "flex-start" }}>
                    <button type="button" onClick={handleCopy} style={{ minHeight: 46, padding: "0 16px", borderRadius: 14, border: "1px solid var(--line-strong)", background: "#fff", color: "var(--ink)", fontSize: 14, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
                      <Copy size={15} />
                      {copied ? "Lien copie" : "Copier le lien"}
                    </button>
                    <a href={getMarketingAuditPdfUrl(audit.shareToken)} target="_blank" rel="noreferrer" style={{ minHeight: 46, padding: "0 16px", borderRadius: 14, border: "1px solid var(--ink)", background: "var(--ink)", color: "#fff", fontSize: 14, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 8, cursor: "pointer", textDecoration: "none" }}>
                      <Download size={15} />
                      Exporter en PDF
                    </a>
                  </div>
                ...
                {!isReady ? (
                  <>
                    <div style={{ marginTop: 28, display: "grid", gap: 18, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
                      {[
                        { label: "Statut", value: getAuditStageLabel(audit.status), hint: "Le score gratuit apparait uniquement apres connexion de la vraie source.", accent: "#2A6FE8" },
                        { label: "Source attendue", value: audit.cmsUsed || audit.connectorType, hint: "Source que FeedPlug doit connecter pour lancer l analyse.", accent: "var(--accent)" },
                        { label: "Canaux cibles", value: audit.targetChannels.length, hint: audit.targetChannels.join(", "), accent: "var(--success)" },
                        { label: "Catalogue declare", value: audit.catalogSize, hint: "Volume annonce pour preparer l audit.", accent: "var(--accent)" },
                      ].map((card) => (
                        <div key={card.label} className="audit-card" style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 22, background: "#fff" }}>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "var(--ink-3)" }}>{card.label}</p>
                          <p style={{ margin: "12px 0 8px", fontSize: 30, lineHeight: 1, fontWeight: 700, letterSpacing: "-0.05em", color: card.accent }}>{card.value}</p>
                          <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: "var(--ink-2)" }}>{card.hint}</p>
                        </div>
                      ))}
                    </div>

                    <div className="audit-cta" style={{ marginTop: 28, padding: 24, borderRadius: 24, background: "var(--paper-2)", border: "1px solid var(--line)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 18, flexWrap: "wrap", alignItems: "center" }}>
                        <div style={{ maxWidth: 760 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            <CheckCircle2 size={18} color="#2A6FE8" />
                            <span style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#2A6FE8" }}>Prochaine etape</span>
                          </div>
                          <h2 style={{ margin: "0 0 8px", fontSize: 22, color: "var(--ink)" }}>Connectez une vraie source pour declencher l audit technique gratuit.</h2>
                        ...
                        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                          <button type="button" onClick={handleShopifyConnect} disabled={connecting !== "" || shopifyConnected} style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "#fff", border: "1px solid var(--line-strong)", color: "var(--ink)", display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", fontSize: 14, fontWeight: 700, cursor: connecting ? "wait" : "pointer", opacity: shopifyConnected ? 0.6 : 1 }}>
                            {shopifyConnected ? "Shopify connecte" : connecting === "shopify" ? "Connexion..." : "Connecter Shopify"}
                          </button>
                          <button type="button" onClick={handleGmcConnect} disabled={connecting !== "" || gmcConnected} style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "#fff", border: "1px solid var(--line-strong)", color: "var(--ink)", display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", fontSize: 14, fontWeight: 700, cursor: connecting ? "wait" : "pointer", opacity: gmcConnected ? 0.6 : 1 }}>
                    ...
                    <div style={{ marginTop: 18, display: "grid", gap: 16, gridTemplateColumns: "minmax(0, 1fr) minmax(300px, 0.9fr)" }}>
                      <div className="audit-card" style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 24, background: "#fff" }}>
                        <h2 style={{ margin: "0 0 16px", fontSize: 20, color: "var(--ink)" }}>Ce qui est inclus gratuitement</h2>
                        <div style={{ display: "grid", gap: 12 }}>
                          {[
                            ...
                            <div key={item} style={{ display: "flex", alignItems: "center", gap: 10, color: "var(--ink-2)", fontSize: 14 }}>
                              <CheckCircle2 size={16} color="#2A6FE8" />
                              <span>{item}</span>
                            </div>
                          ))}
                        </div>
                    ...
                    {(audit.connectorType === "PRESTASHOP" || (audit.cmsUsed || "").toLowerCase().includes("presta")) ? (
                      <div style={{ marginTop: 18 }} className="audit-card">
                        <div style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 24, background: "#fff" }}>
                          <h2 style={{ margin: "0 0 12px", fontSize: 20, color: "var(--ink)" }}>Connecter PrestaShop</h2>
                          <p style={{ margin: "0 0 16px", fontSize: 14, lineHeight: 1.7, color: "var(--ink-2)" }}>
                          ...
                          <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
                            <input
                              ...
                              style={{ minHeight: 48, borderRadius: 14, border: "1px solid var(--line-strong)", padding: "0 14px", fontSize: 14, color: "var(--ink)" }}
                            />
                            <input
                              type="password"
                          ...
                          <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                            <button
                              ...
                              style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "var(--ink)", border: "1px solid var(--ink)", color: "#fff", display: "inline-flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 700, cursor: connecting ? "wait" : "pointer", opacity: prestashopConnected ? 0.6 : 1 }}
                            >
                    ...
                    {(audit.connectorType === "CSV" || (audit.cmsUsed || "").toLowerCase().includes("csv") || (audit.cmsUsed || "").toLowerCase().includes("xml")) ? (
                      <div style={{ marginTop: 18 }} className="audit-card">
                        <div style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 24, background: "#fff" }}>
                          <h2 style={{ margin: "0 0 12px", fontSize: 20, color: "var(--ink)" }}>Connecter un flux CSV / XML</h2>
                          <p style={{ margin: "0 0 16px", fontSize: 14, lineHeight: 1.7, color: "var(--ink-2)" }}>
                          ...
                          <input
                            ...
                            style={{ width: "100%", minHeight: 48, borderRadius: 14, border: "1px solid var(--line-strong)", padding: "0 14px", fontSize: 14, color: "var(--ink)" }}
                          />
                          <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                            <button
                              type="button"
                              ...
                              style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "var(--ink)", border: "1px solid var(--ink)", color: "#fff", display: "inline-flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 700, cursor: connecting ? "wait" : "pointer", opacity: fileConnected ? 0.6 : 1 }}
                            >
                  ...
                  <>
                    <div style={{ marginTop: 28, display: "grid", gap: 18, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
                      {[
                        { label: "Score actuel", value: `${audit.report?.score}/100`, hint: "Etat technique du flux aujourd hui", accent: "#2A6FE8" },
                        { label: "Potentiel avec FeedPlug", value: `${audit.report?.potentialScore}/100`, hint: "Score projetable apres correction des points critiques", accent: "var(--accent)" },
                        { label: "Produits recuperables", value: audit.report?.estimatedAdditionalApprovedProducts ?? 0, hint: "Produits additionnels potentiellement diffusables", accent: "var(--success)" },
                        { label: "Gain de visibilite estime", value: `${audit.report?.estimatedVisibilityLiftPct ?? 0}%`, hint: "Projection basee sur l analyse technique reelle", accent: "var(--accent)" },
                      ].map((card) => (
                        <div key={card.label} className="audit-card" style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 22, background: "#fff" }}>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "var(--ink-3)" }}>{card.label}</p>
                    ...
                    {audit.report?.scoreBand ? (
                      <div style={{ marginTop: 18, padding: 22, borderRadius: 22, border: "1px solid var(--accent-bg)", background: "#f8fbff" }}>
                        ...
                        <h2 style={{ margin: "0 0 8px", fontSize: 22, color: "var(--ink)" }}>{audit.report.scoreBand.label}</h2>
                        <p style={{ margin: 0, fontSize: 14, lineHeight: 1.7, color: "var(--ink-2)" }}>{audit.report.scoreBand.description}</p>
                      </div>
                    ) : null}

                    {audit.report?.auditPillars?.length ? (
                      <div style={{ marginTop: 18 }}>
                        <h2 style={{ margin: "0 0 16px", fontSize: 22, color: "var(--ink)" }}>4 leviers de valeur</h2>
                        <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
                          {audit.report.auditPillars.map((pillar) => (
                            <div key={pillar.key} className="audit-card" style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 22, background: "#fff" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 10 }}>
                                <span style={{ fontSize: 15, fontWeight: 700, color: "var(--ink)" }}>{pillar.label}</span>
                                <span style={{ fontSize: 18, fontWeight: 700, color: "#2A6FE8" }}>{pillar.score}/100</span>
                              </div>
                              <div style={{ height: 10, borderRadius: 999, background: "var(--line)", overflow: "hidden", marginBottom: 10 }}>
                                <div style={{ width: `${Math.max(4, Math.min(pillar.score, 100))}%`, height: "100%", borderRadius: 999, background: "#2A6FE8" }} />
                              </div>
                              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.7, color: "var(--ink-2)" }}>{pillar.detail}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div style={{ marginTop: 28, display: "grid", gap: 18, gridTemplateColumns: "minmax(0, 1fr) minmax(300px, 0.9fr)" }}>
                      <div className="audit-card" style={{ border: "1px solid var(--line)", borderRadius: 22, padding: 24, background: "#fff" }}>
                        <h2 style={{ margin: "0 0 16px", fontSize: 20, color: "var(--ink)" }}>Breakdown du score</h2>
                        {[
                          { label: "Couverture des donnees", value: audit.report?.scoreBreakdown.dataCoverage ?? 0, accent: "#2A6FE8" },
                          { label: "Qualite produit moyenne", value: audit.report?.scoreBreakdown.productQuality ?? 0, accent: "var(--accent)" },
                          { label: "Readiness canal", value: audit.report?.scoreBreakdown.channelReadiness ?? 0, accent: "var(--accent)" },
                          { label: "Produits deja prets", value: audit.report?.summary.approvalReadyRate ?? 0, accent: "var(--success)", suffix: "%" },
                        ].map((item) => (
                          <div key={item.label} style={{ marginBottom: 16 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}>
                              <span style={{ fontSize: 14, fontWeight: 600, color: "var(--ink-2)" }}>{item.label}</span>
                        ...
                        <p style={{ margin: "18px 0 0", fontSize: 13, lineHeight: 1.7, color: "var(--ink-3)" }}>{audit.report?.methodology.estimation}</p>
                      </div>
                    ...
                    <div style={{ marginTop: 28 }}>
                      <h2 style={{ margin: "0 0 16px", fontSize: 22, color: "var(--ink)" }}>5 priorites a traiter</h2>
                            ...
                            <div key={issue.key} className="audit-card" style={{ border: `1px solid ${style.border}`, borderRadius: 22, padding: 22, background: style.bg }}>
                              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
                                <div>
                                  ...
                                  <h3 style={{ margin: "10px 0 0", fontSize: 18, color: "var(--ink)" }}>{issue.label}</h3>
                                </div>
                              ...
                              <p style={{ margin: "0 0 8px", fontSize: 14, lineHeight: 1.7, color: "var(--ink-2)" }}>{issue.affectedProducts} produits concernes. {issue.impact}</p>
                              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.7, color: "var(--ink-2)" }}>Action recommandee: {issue.recommendation}</p>
                            </div>
                          );
                        })}
                      </div>
                ...
                <div style={{ marginTop: 18, display: "flex", gap: 12, flexWrap: "wrap", color: "var(--ink-3)", fontSize: 13 }}>
                  <span>Source: {audit.cmsUsed || audit.connectorType}</span>
                  {audit.shopUrl ? <a href={audit.shopUrl} target="_blank" rel="noreferrer" style={{ color: "var(--accent)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}><ExternalLink size={14} />{audit.shopUrl}</a> : null}
                  {audit.merchantId ? <span>Merchant ID: {audit.merchantId}</span> : null}
                </div>

                <div style={{ marginTop: 20, display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <Link href={`/register?email=${encodeURIComponent(audit.email)}`} style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "var(--ink)", color: "#fff", display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", fontSize: 14, fontWeight: 700 }}>
                    Creer mon espace
                    <ArrowRight size={15} />
                  </Link>
                  <Link href={`/${locale}/audit-flux`} style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "#fff", border: "1px solid var(--line-strong)", color: "var(--ink)", display: "inline-flex", alignItems: "center", gap: 8, textDecoration: "none", fontSize: 14, fontWeight: 700 }}>
                    <Share2 size={15} />
                    Nouveau diagnostic
                  </Link>
                  {!isReady ? (
                    <span style={{ minHeight: 48, padding: "0 18px", borderRadius: 14, background: "#fff", border: "1px solid var(--line-strong)", color: "var(--ink-2)", display: "inline-flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 700 }}>
                      <Link2 size={15} />
