  ...
  return (
    <PageLayout style={{ maxWidth: '800px' }}>
      ...
      <PageCard style={{ marginBottom: '24px', padding: '24px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'var(--ink)', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Target style={{ width: '20px', height: '20px', color: 'var(--accent)' }} />
          Effectifs
        </h3>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ padding: '16px', backgroundColor: 'var(--accent-bg)', borderRadius: '8px', borderLeft: '4px solid var(--accent)' }}>
            <p style={{ fontSize: '12px', color: 'var(--accent-2)', fontWeight: '600', margin: '0 0 4px' }}>BRAS TÉMOIN (control)</p>
            <p style={{ fontSize: '24px', fontWeight: '700', color: '#0c4a6e', margin: 0 }}>{controlCount}</p>
            <p style={{ fontSize: '13px', color: 'var(--ink-3)', margin: '4px 0 0' }}>Titre original conservé</p>
          </div>
          <div style={{ padding: '16px', backgroundColor: 'var(--accent-bg)', borderRadius: '8px', borderLeft: '4px solid var(--accent)' }}>
            <p style={{ fontSize: '12px', color: 'var(--accent-2)', fontWeight: '600', margin: '0 0 4px' }}>BRAS VARIANT</p>
            <p style={{ fontSize: '24px', fontWeight: '700', color: '#4c1d95', margin: 0 }}>{variantCount}</p>
            <p style={{ fontSize: '13px', color: 'var(--ink-3)', margin: '4px 0 0' }}>Titre optimisé IA</p>
          </div>
        </div>
        {test.prerequisitesMet === false && (
          <div style={{ marginTop: '16px', padding: '12px 16px', backgroundColor: 'var(--warning-bg)', borderRadius: '8px', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
            <AlertTriangle style={{ width: '20px', height: '20px', color: 'var(--warning)', flexShrink: 0 }} />
      ...
      {(test.startDate || test.endDate) && (
        <PageCard style={{ marginBottom: '24px', padding: '24px' }}>
          <h3 style={{ fontSize: '16px', fontWeight: '600', color: 'var(--ink)', margin: '0 0 12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Calendar style={{ width: '20px', height: '20px', color: 'var(--ink-3)' }} />
            Durée du test
          </h3>
      ...
      <PageCard style={{ marginBottom: '24px', padding: '24px' }}>
        ...
        <div style={{ padding: '16px', backgroundColor: 'var(--paper-2)', borderRadius: '8px', borderLeft: '4px solid var(--accent)' }}>
        ...
        {resultSummary && typeof resultSummary === 'object' && Object.keys(resultSummary).length > 0 && (
          <details style={{ marginTop: '16px' }}>
            <summary style={{ fontSize: '13px', color: 'var(--ink-3)', cursor: 'pointer' }}>Voir le détail des métriques</summary>
            <pre style={{ marginTop: '8px', padding: '12px', backgroundColor: 'var(--paper-2)', borderRadius: '6px', fontSize: '12px', overflow: 'auto' }}>
