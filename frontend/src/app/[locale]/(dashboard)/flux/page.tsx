  ...
  return (
    <PageLayout>
      {gmcSelection && (
        <PageCard
          style={{
            marginBottom: '20px',
            borderColor: '#99f6e4',
            background: 'linear-gradient(135deg, #E8EFFB 0%, var(--paper-2) 100%)',
          ...
          <div style={{ display: 'grid', gap: '18px' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ display: 'grid', gap: '6px' }}>
                <p style={{ margin: 0, fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#2A6FE8' }}>
                  Etape requise
                </p>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      textAlign: 'left',
                      borderRadius: '16px',
                      border: selected ? '1.5px solid #2A6FE8' : '1px solid var(--line-strong)',
                      backgroundColor: selected ? '#E8EFFB' : '#ffffff',
                      padding: '14px 16px',
                      cursor: 'pointer',
                      display: 'grid',
                      gap: '4px',
      ...
      {pushResult && (() => {
        ...
        const borderColor = allFailed ? '#fca5a5' : hasErrors ? 'var(--warning)' : hasNoProducts ? 'var(--line-strong)' : '#BBF7D0';
            ...
            <>
              ...
              <PageCard style={{ marginTop: '20px', border: '1px solid rgba(226,232,240,0.9)', boxShadow: '0 10px 28px rgba(15,23,42,0.04)' }}>
                <div style={{ display: 'grid', gap: '18px', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
                  {[
                    { label: 'Couverture des donnees', value: feedAudit.scoreBreakdown.dataCoverage, tone: '#2A6FE8' },
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  <div style={{ display: 'flex', gap: '8px', position: 'relative', flexWrap: 'wrap' }}>
                      ...
                      {exportMenuFeedId === feed.id && (
                        <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: '4px', background: 'white', border: '1px solid var(--line)', borderRadius: '6px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 50, minWidth: '260px', maxHeight: '70vh', overflowY: 'auto', padding: '4px 0' }}>
                          ...
                          {AMAZON_EXPORT_CHANNELS.map((ch) => (
                            <button key={ch.channelKey} type="button" onClick={() => handleExportCsvAmazon(feed.id, ch.channelKey)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', border: 'none', background: 'none', fontSize: '13px', cursor: 'pointer', color: 'var(--ink)' }}>
                              {ch.label}
                            </button>
                          ))}
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        {amazonPushMenuFeedId === feed.id && (
                          <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: '4px', background: 'white', border: '1px solid var(--line)', borderRadius: '6px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', zIndex: 50, minWidth: '160px', padding: '4px 0' }}>
                            {AMAZON_EXPORT_CHANNELS.map((ch) => (
                              <button key={ch.channelKey} type="button" onClick={() => handlePushAmazon(feed.id, ch.channelKey)} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', border: 'none', background: 'none', fontSize: '13px', cursor: 'pointer', color: 'var(--ink)' }}>
                                {ch.label}
                              </button>
                            ))}
                          </div>
                    ...
                    <button
                      ...
                      style={{
                        border: selectedAuditFeedId === feed.id ? '1px solid #2A6FE8' : '1px solid var(--line-strong)',
                        color: selectedAuditFeedId === feed.id ? '#2A6FE8' : 'var(--ink-2)',
                        padding: '6px 12px',
                        borderRadius: '10px',
                        fontSize: '12px',
                        display: 'flex',
